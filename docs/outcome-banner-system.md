...
6. Игрок нажимает `Continue`.
7. Запускается телепортация персонажа и появление следующего персонажа.

Поток неправильного ответа:

1. Игрок выбирает ответ в `QuestPanel`.
2. `main_quest_logic.gd` определяет индекс ответа.
3. По этому индексу сразу берётся картинка из `current_question.answer_banners`.
4. `QuestPanel` закрывается.
5. Открывается `CanvasLayer/UIRoot/OutcomePanel`.
6. Игрок нажимает `Continue`.
7. Запускается провал, скрытие персонажа и плохой портал.
...
- первый ответ правильный, даёт `Good +1`, показывает `v-p-tower.png`, затем запускает телепортацию;
